...
import { markErrorInteracted, isErrorInteracted, cleanupStaleEntries } from '@/utils/errorCardState'
...
export function ErrorCard({
  ...
  const [isInteracted, setIsInteracted] = useState(() => {
    if (!isLastErrorMessage) return true
    if (!subtaskId) return false
    cleanupStaleEntries()
    return isErrorInteracted(subtaskId)
  })
  ...
  const handleInteraction = useCallback(
    (action: string) => {
      if (subtaskId) markErrorInteracted(subtaskId, action)
